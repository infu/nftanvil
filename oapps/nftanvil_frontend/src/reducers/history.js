...
export const loadInfo = () => async (dispatch, getState) => {
  ...
  let history = historyCanister(
    PrincipalFromSlot(s.ic.map.space, s.ic.map.history),
  ...
  let p = {
    total,
    canister: PrincipalFromSlot(s.ic.map.space, s.ic.map.history).toText(),
  };
  ...
  async (dispatch, getState) => {
    ...
    let r = await Promise.all(
      transactions.map(async (tx_id) => {
        ...
        let canister = PrincipalFromSlot(s.ic.map.space, slot);
