...
import Ledger  "mo:anvil/type/ledger_interface";
...
shared({caller = _installer}) actor class Class() : async IF.Interface = this {
  ...
  public shared({caller}) func nft_mint(slot: Nft.CanisterSlot, request: Nft.MintRequest) : async Nft.MintResponse {
    ...
    let scriptAccount = getScriptAccount();
    let pwr = Cluster.pwr(anvil.conf, Cluster.pwr2slot(anvil.conf, scriptAccount));

    let response = await pwr.nft_mint(slot, request);

    switch (response) {
      case (#ok(x)) {
        let {tokenIndex} = x;

        let nft_id = Nft.TokenIdentifier.encode(slot, tokenIndex);
  ...
  public shared({caller}) func burn(request: Nft.BurnRequest) : async Nft.BurnResponse {
    ...
    let nft = Cluster.nftFromTid(anvil.conf, request.token);

    let response = await nft.burn(request);
  ...
  private func add_one(nft_id: Nft.TokenIdentifier) : () {
   label lo loop {
  ...
  public shared({caller}) func icp_balance() : async Result.Result<Nft.Balance, Text> {
    ...
    // let (purchaseAccountId, purchaseSubAccount) = Nft.AccountIdentifier.purchaseAccountId(Principal.fromActor(this), 0, scriptAccount);
    // return #err(Nft.AccountIdentifier.toText(purchaseAccountId));

    // ignore await icp_fix();

    ignore await icp_claim();
   
    ignore await Cluster.pwrFromAid(anvil.conf, scriptAccount).pwr_purchase_claim({
                  user = #address(scriptAccount);
                  subaccount = null
                });
  ...
  // private func icp_fix() : async Result.Result<(), Text> {
  //       let scriptAccount = getScriptAccount();
  ...
  //       if (payment <= anvil.oracle.icpFee) return #ok();

  //       let amount = {e8s = payment - anvil.oracle.icpFee};

  //       let transfer : Ledger.TransferArgs = {
  //           memo = 0;
  //           amount;
  //           fee = {e8s = anvil.oracle.icpFee};
  ...
  private func icp_claim() : async Result.Result<(), Text> {
        let scriptAccount = getScriptAccount();

        let pwrPrincipal = Nft.APrincipal.fromSlot(anvil.conf.space, Cluster.pwr2slot(anvil.conf, scriptAccount));
        let (purchaseAccountId, purchaseSubAccount) = Nft.AccountIdentifier.purchaseAccountId(pwrPrincipal, 0, scriptAccount);

        let {e8s = payment} = await ledger.account_balance({
            account = scriptAccount
        });

        if (payment <= anvil.oracle.icpFee) return #err("Noting in account " # debug_show(payment) # " acc:" # Nft.AccountIdentifier.toText(scriptAccount));
