  ...
  reducers: {
    nftSet: (state, action) => {
      return produce(state, (draft) => {
        draft[action.payload.id] = action.payload.meta;
        if (state[action.payload.id])
          draft[action.payload.id].busy = state[action.payload.id].busy;
        return draft;
      });
    },
    setBusy: (state, action) => {
      return produce(state, (draft) => {
        draft[action.payload.id].busy = action.payload.busy;
        return draft;
      });
    },
  },
...
export const { nftSet, setBusy } = nftSlice.actions;
